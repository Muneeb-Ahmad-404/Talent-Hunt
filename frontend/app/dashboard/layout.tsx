...
const recruiterNav = [['Overview', '/dashboard'], ['Company mode', '/dashboard/company'], ['Jobs', '/dashboard/jobs'], ['Hiring pipeline', '/dashboard/applicants'], ['Post a job', '/dashboard/jobs/new'], ['Team members', '/dashboard/members']];
...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    Promise.all([
      ...
      fetch('/api/companies/me', { credentials: 'include' }).then((r) => r.ok ? r.json() : null),
    ]).then(([auth, company]) => {
      const user = auth?.user ?? auth ?? {};
      const companyData = company?.company ?? company ?? {};
      setRole(String(companyData.companyRole ?? companyData.role ?? user.role ?? '').toLowerCase());
    });
  }, []);
  const isApplicant = role === 'applicant' || role === 'candidate';
  const isCompanyMember = ['owner', 'hr_manager', 'recruiter', 'hiring_manager'].includes(role);
  const nav = isApplicant ? applicantNav : isCompanyMember ? recruiterNav : [];
